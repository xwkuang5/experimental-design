...
def parse_config(config):
    ...
    for task_name in available_tasks:
        if available_tasks[task_name]['repeatable']:
            ...
            print("[INFO] Found one repeatable task: {}, skipping all other tasks.".format(task_name))
            break
        else:
    ...
    for variable_name in independent_variables:
        try:
            ...
            if factor_type == FactorType.between_subject.name:
                factors_list += [Factor(name=variable_name, levels=independent_variables[variable_name]['levels'], design=FactorType.between_subject.name)]
            elif factor_type == FactorType.within_subject.name:
                factors_list += [Factor(name=variable_name, levels=independent_variables[variable_name]['levels'], design=FactorType.within_subject.name, order=independent_variables[variable_name]['order'])]
